...
server.tool("update-task", {
    ...
    if (dueDate)
        console.debug(`New due date: ${dueDate}`);
    if (assignee)
        console.debug(`New assignee: ${assignee}`);
    if (completed !== undefined)
        console.debug(`Completed: ${completed}`);
    try {
        if (asanaApi) {
            // Call the actual Asana API
            const updatedFields = {};
...
server.tool("create-section", {
    ...
    if (insertBefore)
        console.debug(`Insert before: ${insertBefore}`);
    if (insertAfter)
        console.debug(`Insert after: ${insertAfter}`);
    try {
        if (asanaApi) {
            // Call the actual Asana API
...
server.tool("add-task-to-section", {
    sectionId: z.string(),
    taskId: z.string()
...
server.tool("remove-dependencies", {
    taskId: z.string(),
    dependencyIds: z.array(z.string())
}, async ({ taskId, dependencyIds }) => {
...
server.tool("get-dependencies", {
    ...
    try {
        if (asanaApi) {
            ...
            const dependencies = result.data.map((dep) => ({
                id: dep.gid,
                name: dep.name,
